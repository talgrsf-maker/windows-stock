...
בקובץ `.env` המקומי צריך רק `VITE_DATA_MODE=google_sheets`. מצב זה יעבוד לאחר הפריסה ל־Vercel, משום שהפרוקסי נמצא בתיקייה `api/`. לפרסום: העלו את התיקייה ל־GitHub, ייבאו את המאגר ב־Vercel, הוסיפו את שני משתני השרת לעיל ופרסמו. בדקו תחילה דיווח אחד ממחשב ושני מטלפון.
